...
def get_toys_values(collection='Marvel'):
    ...
    for toy in toys:
        ...
        toy_attributes_div = toy.find('div', {'data-test': 'product-leaf-attributes-row'})
        if toy_attributes_div is not None:
            toy_data = [item.text.strip() for item in toy_attributes_div.find_all('span')]
...
def main():
    ...
    # soup = get_soup(url='https://www.lego.com/pl-pl/themes/disney')
